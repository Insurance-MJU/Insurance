...
public class UW01ContractReview {
    ...
    public UW01ContractReview(SubscriptionList subscriptionList, ContractList contractList,
                              RiskAnalysisReportList riskReportList, CreditList creditList) {
    ...
    public void run() {
        ...
        // Step 2: 심사 대기 목록 출력
        SubscriptionList pendingList = subscriptionList.findPendingReview();
        System.out.println("\n[ 심사 대기 중인 청약 목록 ]");
        System.out.println("------------------------------------------------------------");
        System.out.printf(" %-20s %-10s %-25s %-12s %-12s %-10s%n",
            "청약번호", "청약자명", "상품명", "보험료", "청약일자", "상태");
        System.out.println("------------------------------------------------------------");
        for (Subscription s : pendingList.getAll()) {
            System.out.printf(" %-20s %-10s %-25s %-12s %-12s %-10s%n",
        ...
        Subscription sub = subscriptionList.findByNo(appNo);
        if (sub == null) {
        ...
        sc.nextLine();
        new UW02RiskAnalysis(riskReportList, creditList).runAsInclude(sub);
