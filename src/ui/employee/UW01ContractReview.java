...
public class UW01ContractReview {
    ...
    public void run() {
        ...
        // Step 2: 심사 대기 목록 출력
        List<Subscription> pendingList = SubscriptionRepository.findPendingReview();
        System.out.println("\n[ 심사 대기 중인 청약 목록 ]");
        System.out.println("------------------------------------------------------------");
        ...
        Subscription sub = SubscriptionRepository.findByNo(appNo);
        if (sub == null) {
        ...
        // Step 6: 위험 분석 보고서 출력
        RiskAnalysisReport report = RiskAnalysisRepository.findBySubscriptionNo(sub.getSubscriptionNo());
        if (report == null) {
